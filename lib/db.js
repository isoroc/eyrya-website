...
export async function createProduct(db, product) {
  const sql = `
    INSERT INTO products (
      slug, name, description, short_description, tagline,
      price, original_price, category, stock, status,
      is_bestseller, rating, reviews, sold,
      images, specs, features, faq
    )
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;
...
export async function updateProduct(db, slug, product) {
  ...
  if (product.tagline !== undefined) { fields.push('tagline = ?'); values.push(product.tagline); }
  ...
  if (product.is_bestseller !== undefined) { fields.push('is_bestseller = ?'); values.push(product.is_bestseller); }
  if (product.rating !== undefined) { fields.push('rating = ?'); values.push(product.rating); }
  if (product.reviews !== undefined) { fields.push('reviews = ?'); values.push(product.reviews); }
  if (product.sold !== undefined) { fields.push('sold = ?'); values.push(product.sold); }
  if (product.images !== undefined) { fields.push('images = ?'); values.push(product.images); }
  if (product.specs !== undefined) { fields.push('specs = ?'); values.push(product.specs); }
  if (product.features !== undefined) { fields.push('features = ?'); values.push(product.features); }
  if (product.faq !== undefined) { fields.push('faq = ?'); values.push(product.faq); }
